...
| **Automation** | [`automated-dev-pipeline`](./Automation/automated-dev-pipeline/SKILL.md) | Scheduled AI agents that do development work unattended. Ideas become scored plans, plans become private repos built one roadmap item per PR, and a finished project goes public on a GitHub account or organization only after scripted gates and a human approval. Refresh and upkeep jobs then keep every repo current with its dependencies, platforms and standards. Also covers the single-repo variant, where stages are folders and a site-deploying merge is the publish step held for a person, and audits such a fleet for cost, overlap and safety. |
