...
> [!NOTE]
> **Keep synced — `org-dev-pipeline`.** Added 2026-09-23. Its design rules, stage contracts and audit checklist were written from the live `ai-automation-tools` pipeline (stage prompts and runner in `~/.claude/routines/jobs/_org-pipeline/`, publish and security scripts in `GitHub-Org-Security/`). That pipeline's paths and parameter values are deliberately **not** in this repo: they're in `~/.claude/routines/jobs/_org-pipeline/reference-implementation.md`, which the skill points to. **This repo's copy is canonical.** Edit it here and reinstall with `install-skills.ps1 -Core -Skill org-dev-pipeline`, never in `~/.claude/skills/`, because the install wipes that folder. When the pipeline learns a new rule, fold it into the skill by hand. There is no automatic sync.
